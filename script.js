...
var svg = d3.select('#wheel').append("svg").data([DATA]);
...
//make arrow
svg.append("g").attr("transform", "translate(" + (w + padding.left + padding.right - 15) + "," + ((h / 2) + padding.top) + ")").append("path").attr("d", "M-" + (r * .25) + ",0L0," + (r * .15) + "L0,-" + (r * .18) + "Z").style({ "fill": "#EF233C", "filter": "drop-shadow(0px 4px 4px rgba(0, 0, 0, 0.25))" });
...
async function copyCode() {
    ...
    try {
        ...
        await moengage.copyText((copyText.innerText || '').trim(), "Code copied");
